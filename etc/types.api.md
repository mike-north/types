## API Report File for "@mike-north/types"

> Do not edit this file. It is a report generated by [API Extractor](https://api-extractor.com/).

```ts

// @public
export type AsyncMethodReturns<T, K extends keyof T = keyof T> = {
    [KK in K]: T[KK] extends (...args: any[]) => PromiseLike<any> ? T[KK] : T[KK] extends (...args: infer A) => infer R ? (...args: A) => Promise<R> : T[KK];
};

// @public
export type ConstructorArgs<K extends new (...args: any[]) => any> = K extends new () => any ? never[] : K extends new (a: infer A) => any ? [A] : K extends new (a: infer A, b: infer B) => any ? [A, B] : K extends new (a: infer A, b: infer B, c: infer C) => any ? [A, B, C] : K extends new (a: infer A, b: infer B, c: infer C, d: infer D) => any ? [A, B, C, D] : K extends new (a: infer A, b: infer B, c: infer C, d: infer D, e: infer E) => any ? [A, B, C, D, E] : never;

// @public @deprecated (undocumented)
export type DeepPartial<O> = O extends string | Function | number | boolean ? O : {
    [K in keyof O]?: DeepPartial<O[K]>;
};

// @public
export class Deferred<T = any> {
    constructor();
    readonly promise: PromiseLike<T>;
    static promiseConstructor: PromiseConstructor;
    readonly reject: (reason?: any) => any;
    readonly resolve: (value?: T | PromiseLike<T>) => void;
}

// @public
export interface Dict<T = any> {
    // (undocumented)
    [k: string]: T | undefined;
}

// @public
export type ExtractArgs<F> = F extends (a: infer A, b: infer B, c: infer C, d: infer D, e: infer E) => any ? [A, B, C, D, E] : F extends (a: infer A, b: infer B, c: infer C, d: infer D) => any ? [A, B, C, D] : F extends (a: infer A, b: infer B, c: infer C) => any ? [A, B, C] : F extends (a: infer A, b: infer B) => any ? [A, B] : F extends (a: infer A) => any ? [A] : F extends () => any ? [] : never;

// @public
export type ExtractPropertyNamesOfType<T, S> = {
    [K in keyof T]: T[K] extends S ? K : never;
}[keyof T];

// @public
export function isDefined<T>(arg: T | undefined): arg is T;

// @public
export function isNonNull<T>(arg: T | null): arg is T;

// @public
export function isPresent<T>(arg: T | null | undefined): arg is T;

// @public
export type OptionalPropertyNamesOf<T extends object> = Exclude<{
    [K in keyof T]: T extends Record<K, T[K]> ? never : K;
}[keyof T], undefined>;

// @public
export type OptionalProps<T, K extends keyof T> = {
    [L in K]?: T[L];
} & {
    [M in Exclude<keyof T, K>]: T[M];
};

// @public
export type RequiredPropertyNamesOf<T extends object> = Exclude<{
    [K in keyof T]: T extends Record<K, T[K]> ? K : never;
}[keyof T], undefined>;

// @public
export type RequiredProps<T, K extends keyof T> = T & {
    [L in K]-?: T[K];
};


// (No @packageDocumentation comment for this package)

```
